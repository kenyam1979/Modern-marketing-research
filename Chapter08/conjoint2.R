...
# データ読み込み
dat <- read_tsv('http://yuhikaku-nibu.txt-nifty.com/blog/files/conjoint2.txt', 
                     locale=locale(encoding='shift-jis'),
                     col_names=c('sample_id','card_id', 'wty_2', 'battery_6', 'battery_4', 'color_red', 'color_silver', 'util', 'rank'), 
                     skip=1)
dat

# 回帰分析で係数算出
model <- lm(formula=util~battery_6+battery_4+wty_2+color_red+color_silver, data=dat)
summary(model)
